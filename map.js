document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('japan.svg')
    ...
    .then(svgText => {
      ...
      const groupSettings = {};
      Object.keys(REGION_DB).forEach(gid => groupSettings[gid] = {...REGION_DB[gid].transform, hash: REGION_DB[gid].hash});
      ...
      const prefNames = {
        ...
        SHIZUOKA:'静岡県', YAMANASHI:'山梨県', NAGANO:'長野県', ISHIKAWA:'石川県', TOYAMA:'富山県', FUKUI:'福井県',
        GIFU:'岐阜県', AICHI:'愛知県', MIE:'三重県', NARA:'奈良県', WAKAYAMA:'和歌山県', OSAKA:'大阪府', SHIGA:'滋賀県', KYOTO:'京都府', HYOGO:'兵庫県',
      ...
      prefGroup.querySelectorAll('path').forEach(p => {
        ...
        if(testDiv){
          p.addEventListener('click', e=>{
            const prefId = p.id;
            testDiv.textContent = `クリック: ${prefNames[prefId]} (${prefId})`;
          });
        }
      });

      // ★以降、既存のクリック・拡大・グループ選択などロジックはそのまま維持
      ...
      function createCornerDummy(position){
        ...
        wrapper.style.position='absolute'; wrapper.style.display='none'; wrapper.style.flexDirection='column'; wrapper.style.gap='4px'; wrapper.style.zIndex='10';
      ...
      function createCornerDummyWrapper(vertical,posValue,horPercent,axis){
        ...
        if(vertical==='top'){wrapper.style.top=posValue+'px';} else{wrapper.style.bottom=posValue+'px';}
