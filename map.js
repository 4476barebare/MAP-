...
Object.keys(groupBoxSettings).forEach(gid => {
  groupToPrefectures[gid] = Object.values(groupBoxSettings[gid]).flat();
    ...
    let regionHash = null;
    for (const pathKey in REGION_DB) {
        const boxes = REGION_DB[pathKey].boxes;
        for (const boxKey in boxes) {
            if (boxes[boxKey].includes(prefId)) {
                regionHash = REGION_DB[pathKey].hash;
                break;
            }
        }
        if (regionHash) break;
