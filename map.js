...
function createDebugConsole(){
  ...
  panel.style.border = '1px solid #191970';
  ...
  panel.innerHTML = `
    <div>
      <label>Group: </label>
      <select id="debugGroup">
        <option value="Path_2">Path_2</option>
        <option value="Path_3">Path_3</option>
        <option value="Path_4">Path_4</option>
        <option value="Path_5">Path_5</option>
        <option value="Path_6">Path_6</option>
        <option value="Path_7">Path_7</option>
      </select>
    </div>
    <div>
      <label>X: </label><input type="number" id="debugX" step="1" value="0" style="width:50px;">
    </div>
    <div>
      <label>Y: </label><input type="number" id="debugY" step="1" value="0" style="width:50px;">
    </div>
    <div>
      <label>Scale: </label><input type="number" id="debugScale" step="0.1" value="1" style="width:50px;">
    </div>
  ...
  document.getElementById('debugApply').addEventListener('click', ()=>{
    const g = document.getElementById('debugGroup').value;
    const x = parseFloat(document.getElementById('debugX').value);
    const y = parseFloat(document.getElementById('debugY').value);
    const s = parseFloat(document.getElementById('debugScale').value);

    const group = document.getElementById(g);
    if(!group) return;

    const bbox = group.getBBox();
    const tx = (svg.clientWidth/2) - (bbox.x + bbox.width/2 + x) * s;
    const ty = (svg.clientHeight/2) - (bbox.y + bbox.height/2 + y) * s;

    svg.style.transform = `translate(${tx}px, ${ty}px) scale(${s})`;

    console.log(`${g}: { scale:${s}, x:${x}, y:${y} }`);
  });
}
