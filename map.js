  ...
  fetch('japan.svg')
    ...
    .then(svgText => {
      ...
      const groupBoxSettings = {
        ...
        Path_6: {
          top1: ['Tottori','Shimane','Okayama','Hiroshima','Yamaguchi'], // Top-1
          top2: ['Tottori','Shimane','Okayama','Hiroshima','Yamaguchi'], // Top-2 下段
      ...
      const top1Dummy = createTopDummy('top1');
      const top2Dummy = createTopDummy('top2'); // 下段Top-2
      ...
      function hideAllBoxes(){
        [top1Dummy, top2Dummy, bottomDummy, leftTopDummy, rightBottomDummy, leftBottomDummy, rightTopDummy]
          .forEach(wrapper => {
      ...
      function showBoxes(gid){
        ...
        Object.keys(setting).forEach(pos => {
          ...
          setting[pos].forEach((pid, i) => {
            let box;
            if(pos === 'top1') box = wrapper.children[0].children[i];
            else if(pos === 'top2') {
              box = wrapper.children[0].children[i];
              if(i !== 0) box.style.display = 'none'; // 左1個のみ表示
            }
            else box = wrapper.children[i];
      ...
      function createCornerDummy(position){
        ...
        for(let i=0;i<5;i++){ const box=createBox(); box.style.background=color; wrapper.appendChild(box); }
        mapDiv.appendChild(wrapper);
        return wrapper;
      }
