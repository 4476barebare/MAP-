...
for (let i = 1; i < lines.length; i++) {
    ...
    if (!reg || !parent || !fishName || isNaN(lat) || isNaN(lng)) continue;
    ...
    // 座標（lat, lng）を交互に配列の末尾へ追加
    resultData[reg][parent][fishName].coords.push(lat, lng);

    // ★拡張列のデータを格納（空欄でなければ追加）
    extraColumns.forEach(ext => {
        const val = cols[ext.index] ? cols[ext.index].trim() : '';
        if (val !== '') {
            // 一度だけ格納すればOK（同じスポット・魚種で同じ値を持つ前提）
